...
export const applyStatus = (target, statusId, duration = 3, value = null, context, silent = false) => {
    ...
    if (existing) {
        ...
        // Create full status instance from definition
        const statusInstance = {
            ...statusDef,
            duration: duration,
            value: value,
            sourceId: context?.actor?.id || null,
            targetId: target.id || null,
            uuid: 's' + Math.random().toString(36).substr(2, 9)
        };
        
        target.statusEffects.push(statusInstance);
...
export const removeStatus = (target, statusId, context, silent = false) => {
    ...
    if (idx !== -1) {
        const status = target.statusEffects[idx];
        target.statusEffects.splice(idx, 1);
        if (!silent && log) log('battle.statusCured', { target: target.name, status: status.name });
    }
};
